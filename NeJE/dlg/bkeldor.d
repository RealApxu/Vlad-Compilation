REPLACE_STATE_TRIGGER BKELDOR 26
~InParty("Anomen")
See("Anomen")
!StateCheck("Anomen",STATE_SLEEPING)
Global("KeldornAnomen","LOCALS",1)
...
//LT4
IF WEIGHT #-1 ~Global("VP_CK_Banter","LOCALS",1)
GlobalGT("VP_LoveTalk","LOCALS",1)
GlobalLT("VP_LoveTalk","LOCALS",11)~ THEN BEGIN LT4_st0
...
SetGlobalTimer("KeldornHome","GLOBAL",TWO_DAYS)
SetGlobal("KeldornComplain","LOCALS",1)~ GOTO LT6_st0A
...
SetGlobalTimer("KeldornHome","GLOBAL",TWO_DAYS)
SetGlobal("KeldornComplain","LOCALS",1)~ GOTO LT6_st0B
...
SetGlobalTimer("KeldornHome","GLOBAL",TWO_DAYS)
SetGlobal("KeldornComplain","LOCALS",1)~ GOTO LT6_st0C
END
...
IF ~~ THEN BEGIN LT12_st23
	SAY ~(Laughs) I will enjoy being sick.~
		IF ~~ THEN DO ~SetGlobal("VP_Kr_Messenger","LOCALS",1)
SetGlobalTimer("VP_Kr_Mess_Timer","GLOBAL",ONE_DAY)
SetGlobal("VP_AnomenJealous","GLOBAL",1)~  EXIT
END
...
IF ~~ THEN BEGIN LT26_st6
	SAY ~There is no need for embarrassment, you sat by me, remember and now it is my turn. Take off your clothes, I'll look away if it makes you feel better. Now sleep, there is nothing for you to fear.~
		IF ~~ THEN DO ~SetGlobal("VP_LoveTalk","LOCALS",27)
ClearAllActions()
StartCutSceneMode()
Wait(4)
RestParty()
...
IF ~~ THEN BEGIN LT26_st8
	SAY ~So what... you were not naked. And even if you were, would it be so terrible? Now, get dressed. (He turns away, allowing you the privacy to get dressed.)~
		IF ~~ THEN DO ~SetGlobal("VP_MorningTalk","LOCALS",2)
...
IF ~~ THEN BEGIN LT30_st13
	SAY ~(You sit a while longer and eventually you fall asleep. As your head touches the pillow, your eyes close and you do not hear Keldorn wishing you sweet dreams. In front of you a strange face appears, strange and yet you sense that it is a face known to you.)~
		IF ~~ THEN DO ~SetGlobal("VP_LoveTalk","LOCALS",31)
SetGlobal("VP_Kr_AfterDream","LOCALS",1)
SetGlobalTimer("VP_KrRomance_Timer","GLOBAL",ONE_DAY)
ClearAllActions()
StartCutSceneMode()
StartCutScene("MovieKr")~ EXIT
END

IF WEIGHT #-13 ~Global("VP_LoveTalk","LOCALS",31)
Global("VP_Kr_AfterDream","LOCALS",1)~ THEN BEGIN AftDream
...
IF WEIGHT #-15 ~Global("Kr_ElfBanter","LOCALS",2)~ THEN BEGIN ElfTlk0
	SAY ~<CHARNAME>, <CHARNAME>... what ever is it? You are shaking all over... (putting his arms around you). Please calm yourself... we are at war and elves have never been known to trust strangers at the best of times.~
		IF ~!Race(Player1,ELF)
!Race(Player1,HALF_ELF)~ THEN REPLY ~It's just... this damned interrogation Keldorn, it is absolutely revolting!~ GOTO ElfTlk1
...
IF ~~ THEN BEGIN LT34_st14
	SAY ~(His voice becomes flat) You are mistaken, my love. Will you not leave me even a little hope? I can't bear this... (Pressing his hand to his chest) I won't.~
		IF ~~ THEN DO ~SetGlobal("Kr_LakeBanter","LOCALS",3)
...
IF ~~ THEN BEGIN TK_st1
	SAY ~Oh, of course the dress. It is very important, that I understand. Would you mind if I have a look? I am interested in everything that concerns you and our forthcoming wedding. Besides I am very curious as to what sort of taste this Frida has.~
		IF ~~ THEN DO ~SetGlobal("VP_Castle_Banters","GLOBAL",2)
ClearAllActions()
StartCutSceneMode()
StartCutScene("KRCut22")~ EXIT
END	
...
IF WEIGHT #-19 ~Global("VP_Castle_Banters","GLOBAL",3)
Global("VP_LoveTalk","LOCALS",31)~ THEN BEGIN LT36_st0
...
IF ~~ THEN BEGIN LT36_st30
	SAY ~(Carrying you into his room and... suddenly Slayer appears.)~
		IF ~~ THEN DO ~SetGlobal("VP_LoveTalk","LOCALS",32)
ClearAllActions()
StartCutSceneMode()
StartCutScene("KRCut24")~ EXIT
END
...
RealSetGlobalTimer("VP_KrRomance_Timer","GLOBAL",5)
ClearAllActions()
StartCutSceneMode()
Wait(4)
RestParty()
