...
EXTEND_BOTTOM IMOEN2 21
  IF ~Global("Kachiko_Saved","GLOBAL",2) InParty("Yoshimo") !Dead("Yoshimo") InParty("Kachiko") !Dead("Kachiko")~ THEN DO ~SetGlobal("VP_YoshimoExcuse","GLOBAL",1)~ EXTERN ~KACHIJ~ N32
  IF ~InParty("Keldorn") !Dead("Keldorn") Global("VP_KeldornMatch","GLOBAL",1)~ THEN EXTERN ~KELDORJ~ Imoen0
  IF ~InParty("Hrothgar") !Dead("Hrothgar")~ THEN GOTO hrothg1
  IF ~InParty("Taffic") !Dead("Taffic")~ THEN EXTERN ~TAFFICJ~ immy1
  IF ~InParty("Melora") !Dead("Melora")~ THEN EXTERN ~MELORAJ~ immy1
  IF ~InParty("Leina") !Dead("Leina")~ THEN EXTERN ~LEINAJ~ immy1
END
...
IF ~~ THEN BEGIN KachiBye
  SAY ~Bye, Kachiko~
  IF ~~ THEN DO ~SetGlobal("VP_Imoen_Returns","GLOBAL",1) ActionOverride("Yoshimo",LeaveParty()) ActionOverride("Yoshimo",EscapeArea()) ActionOverride("Kachiko",LeaveParty()) ActionOverride("Kachiko",EscapeArea()) AddXPObject(Myself,1000000) JoinParty()~ EXIT
END
