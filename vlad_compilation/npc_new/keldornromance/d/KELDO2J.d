...
IF ~~ THEN BEGIN SB1_st1
  ...
  IF ~~ THEN DO ~SetGlobal("VP_BSarevok","LOCALS",3) SetGlobal("VP_Kr_NextTalk","GLOBAL",1)~ EXIT
END
...
IF ~~ THEN BEGIN LT4TOB_2
  ...
  IF ~~ THEN DO ~SetGlobal("VP_ToB_LoveTalk","LOCALS",3) SetGlobal("VP_Kr_NextTalk","GLOBAL",4)~ EXIT
END
...
IF ~~ THEN BEGIN LT12Tor2
  ...
  IF ~~ THEN DO ~SetGlobal("VP_ToB_LoveTalk","LOCALS",10) ClearAllActions() StartCutSceneMode() StartCutScene("KRCut31")~ EXIT
END
...
IF ~~ THEN BEGIN LT12_Im0
  ...
  IF ~~ THEN DO ~SetGlobal("VP_ToB_LoveTalk","LOCALS",11) SetGlobalTimer("VP_ToB_Kr_Timer","GLOBAL",ONE_DAY)~ EXIT
END
...
IF ~~ THEN BEGIN KRENDs1C
  ...
  IF ~~ THEN DO ~SetGlobal("VP_Kr_Epilogue","GLOBAL",1) ActionOverride("Kelemvo1",DestroySelf())~ EXTERN ~FINSOL01~ 33 //remove Kelemvor
END
