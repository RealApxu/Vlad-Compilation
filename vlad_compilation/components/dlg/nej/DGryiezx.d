...
IF ~Global("VP_Slavers_Fight","GLOBAL",1)~ THEN BEGIN 0
  ...
  IF ~~ THEN DO ~SetGlobal("VP_Slavers_Fight","GLOBAL",2) SetGlobal("VP_Slave_Released","GLOBAL",1) SetGlobal("MAP_GROUP_HOSTILE5","MYAREA",1) Enemy()~ EXIT
END

IF ~NumTimesTalkedTo(0) Global("VP_Slavers_Fight","GLOBAL",0)~ THEN BEGIN 1
  SAY ~Ugh?~
...
IF ~~ THEN BEGIN 2
  ...
  IF ~OR(2) !InParty("Dar") Dead("Dar")~ THEN REPLY ~Wait Gryiezx...~ GOTO 3
  IF ~InParty("Dar") !Dead("Dar")~ THEN EXTERN ~DARJ~ Gryiezx
END
