...
IF ~  Global("VP_Winthrop_In_NeJ","GLOBAL",0)
~ THEN BEGIN 0
...
~ THEN REPLY ~That is outrageous!  What kind of a fool do you take me for?!  Stuff your Inn, and stuff your company!~ GOTO 1
  IF ~  ReactionGT(LastTalkedToBy,NEUTRAL_UPPER)
~ THEN REPLY ~That is outrageous!  What kind of a fool do you take me for?!  Stuff your Inn, and stuff your company!~ GOTO 4
  IF ~  ReactionGT(LastTalkedToBy,HOSTILE_UPPER)
~ THEN REPLY ~You always were the big kidder, Winthrop.  That gets funnier very nearly every time I hear it.  Well, perhaps not quite so often.~ GOTO 2
  IF ~  ReactionLT(LastTalkedToBy,NEUTRAL_LOWER)
~ THEN REPLY ~You always were the big kidder, Winthrop.  That gets funnier very nearly every time I hear it.  Well, perhaps not quite so often.~ GOTO 5
  IF ~  ReactionGT(LastTalkedToBy,NEUTRAL_UPPER)
~ THEN REPLY ~I fear I do not have that kind of entry fee with me.  I suppose I shall return when I do.~ GOTO 4
  IF ~  ReactionLT(LastTalkedToBy,FRIENDLY_LOWER)
~ THEN REPLY ~I fear I do not have that kind of entry fee with me.  I suppose I shall return when I do.~ GOTO 3
END
...
IF ~~ THEN BEGIN 2
  ...
  IF ~~ THEN REPLY ~Sure, what do you have?~ DO ~StartStore("Inn2616",LastTalkedToBy(Myself))
~ EXIT
END
...
IF ~~ THEN BEGIN 4
  ...
  IF ~~ THEN REPLY ~Yes, what do you have?~ DO ~StartStore("Inn2616",LastTalkedToBy(Myself))
~ EXIT
END
...
IF ~~ THEN BEGIN 5
  ...
  IF ~~ THEN REPLY ~Yes, tell me what you have.~ DO ~StartStore("Inn2616",LastTalkedToBy(Myself))
~ EXIT
END

IF ~  !NumTimesTalkedTo(0)
Global("VP_Winthrop_In_NeJ","GLOBAL",1)
~ THEN BEGIN 6
...
Dead("Dar")
!Global("VP_Slave_Released","GLOBAL",2)
~ THEN REPLY ~Curiosity, nostalgia, not really sure Winthrop. I was passing and thought it would be good to drop in, see the old place again.~ GOTO 12
  IF ~  OR(3)
!InParty("Dar")
Dead("Dar")
!Global("VP_Slave_Released","GLOBAL",2)
~ THEN REPLY ~Do I need a reason to return to the only place I have ever called home Winthrop?~ GOTO 12
  IF ~  OR(3)
!InParty("Dar")
Dead("Dar")
!Global("VP_Slave_Released","GLOBAL",2)
~ THEN REPLY ~That is my business and no one else’s. Now leave me be, I need to rest up a while.~ GOTO 13
  IF ~  InParty("Dar")
!Dead("Dar")
Global("VP_Slave_Released","GLOBAL",2)
Global("VP_Winthrop_Fix","GLOBAL",0)~ THEN REPLY ~Probably something and nothing Winthrop. Tell me, you know of any orc or half orc visitors of late?~ GOTO 14
