...
IF ~NumberOfTimesTalkedTo(0)~ THEN BEGIN 0
  ...
  IF ~InParty("Jaheira") !Dead("Jaheira")~ THEN EXTERN ~JAHEIRAJ~ DG2
  IF ~OR(2) !InParty("Jaheira") Dead("Jaheira")~ THEN REPLY ~Well met Erhard. I am <CHARNAME> and I came here to speak with the druids of this grove, I did not expect to meet with one of the Great Druids. What calls you here Erhard?~ GOTO 17
  IF ~OR(2) !InParty("Jaheira") Dead("Jaheira")~ THEN REPLY ~I am <CHARNAME> and well met to you Erhard. I know very little of the ways of the wilds, but even I can see all is not well in this wood. Tell me, what ails this place Erhard?~ GOTO 17
  IF ~OR(2) !InParty("Jaheira") Dead("Jaheira")~ THEN REPLY ~I am <CHARNAME>. I am here to find an item that I believe is in the keeping of the druids of this grove, if you can call this sickly place a grove. For a Great Druid, I must say you have let this place go a bit Erhard.~ GOTO 17
END
...
IF ~~ THEN BEGIN 13a
  ...
  IF ~InParty("Dar") !Dead("Dar")~ THEN EXTERN ~DARJ~ quest31
  IF ~OR(2) !InParty("Dar") Dead("Dar")~ THEN DO ~SetGlobal("VP_Dar_Quest","GLOBAL",89) EscapeArea()~ EXIT
END
...
IF ~Global("VP_Dar_Quest","GLOBAL",10) Global("VP_Rt_Mine_Visited","GLOBAL",0)~ THEN BEGIN 19
  SAY ~Hello friend.~
  IF ~~ THEN EXIT
END
...
IF ~Global("VP_Rt_Mine_Visited","GLOBAL",2)~ THEN BEGIN 20
  ...
  IF ~Global("SPRITE_IS_DEADIlriana","GLOBAL",0) Global("VP_Dar_Quest","GLOBAL",11)~ THEN REPLY ~I did Erhard. I discovered an abandoned mine under the grove, the entrance of which is close to the pond. In the lower levels of this mine I came across a mage and a group of Svifneblin she had enslaved. They were digging, but for what I did not discover. It is my belief that the renewed digging had released pockets of poisonous vapours seeping into the underground feeds of the pond, poisoning the earth on its way to the pond itself.~ GOTO 22
  IF ~Global("SPRITE_IS_DEADIlriana","GLOBAL",0) Global("VP_Dar_Quest","GLOBAL",10)~ THEN REPLY ~No I did not, I grow tired of the search. I have returned to demand that you give me the book so's I can be gone from here.~ GOTO 25
  IF ~Global("SPRITE_IS_DEADIlriana","GLOBAL",0) Global("VP_Dar_Quest","GLOBAL",10)~ THEN REPLY ~I did not, but I have discovered an entrance to an old mine close by the pool. I suggest that you continue your search there and hand the book over to me as payment for the time I have wasted!~ GOTO 25
  IF ~Global("SPRITE_IS_DEADIlriana","GLOBAL",0) Global("VP_Dar_Quest","GLOBAL",10)~ THEN REPLY ~No, I am here to tell you that I no longer intend to search for this book, nor shall I help you further. Farewell Erhard.~ GOTO 21
  IF ~Global("SPRITE_IS_DEADIlriana","GLOBAL",0) Global("VP_Dar_Quest","GLOBAL",10)~ THEN REPLY ~No, I am here to tell you that I no longer intend to search for this book. My brief look around did uncover an entrance to an old mine close by the pool. I suggest that you continue your search there. Farewell Erhard.~ GOTO 21
END
...
IF ~~ THEN BEGIN 24
  ...
  IF ~~ THEN DO ~SetGlobal("VP_Dar_Quest","GLOBAL",12) GiveItem("DrBook1",Player1) EscapeArea()~ EXIT
END
...
IF ~~ THEN BEGIN 25
  ...
  = ~Now, begone and do not disturb the peace here further.~
  IF ~~ THEN DO ~SetGlobal("VP_Dar_Quest","GLOBAL",90) EscapeArea()~ EXIT
END