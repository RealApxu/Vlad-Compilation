...
IF ~Global("KickedOut","LOCALS",0) Global("VP_In_Dragon_Eye","GLOBAL",1)~ THEN BEGIN 0
...
IF ~Global("KickedOut","LOCALS",0) Global("VP_In_Dragon_Eye","GLOBAL",0)~ THEN BEGIN 3
  ...
  IF ~Global("VP_In_IWD_Part1","GLOBAL",0) Global("VP_InHalruaa","GLOBAL",0) Global("VP_OnIsland","GLOBAL",0) !InWatchersKeep()~ THEN REPLY ~Yes, it must end for now. Safe journey and be well.~ GOTO 2
  IF ~OR(4) Global("VP_In_IWD_Part1","GLOBAL",1) Global("VP_InHalruaa","GLOBAL",1) Global("VP_OnIsland","GLOBAL",1) InWatchersKeep()~ THEN REPLY ~Yes, it must end for now.~ GOTO 5
END
...
IF ~~ THEN BEGIN 6
  ...
  IF ~~ THEN DO ~SetGlobal("VP_Erevain_RaiseDe","GLOBAL",1) ChangeAIScript("",DEFAULT) SetLeavePartyDialogFile() SetGlobal("KickedOut","LOCALS",1) LeaveParty() MoveGlobal("AR0086",Myself,[100.100])~ EXIT
END
...
IF ~HappinessLT(Myself,-299)~ THEN BEGIN 8
  ...
  IF ~~ THEN DO ~SetGlobal("Erevain_Jerk","GLOBAL", 2) LeaveParty() EscapeArea()~ EXIT
END

IF ~~ THEN BEGIN 9
  SAY ~As you wish, <CHARNAME>. I shall be there should you change your mind.~
  IF ~~ THEN DO ~SetGlobal("KickedOut","LOCALS",1) EscapeAreaMove("TT2114",152,150,14)~ EXIT
END

IF WEIGHT #0 ~Global("VP_Erevain_RaiseDe","GLOBAL",3) Global("Erevain_Jerk", "GLOBAL", 2)~ THEN BEGIN 10
  ...
  IF ~~ THEN DO ~SetGlobal("VP_Erevain_RaiseDe","GLOBAL",4) EscapeArea()~ EXIT
END

IF WEIGHT #1 ~Global("VP_Erevain_RaiseDe","GLOBAL",3) !Global("Erevain_Jerk", "GLOBAL", 2)~ THEN BEGIN ErevainRessurection100
  ...
  IF ~OR(2) !Global("YXUNOMEI_DEAD","GLOBAL",0) PartyHasItem("HEARTGM")~ THEN REPLY ~We already defeated the forces in Dragon's Eye.~ GOTO ErevainRessurection115
...
IF ~~ THEN BEGIN ErevainRessurection117
  ...
  IF ~~ THEN DO ~SetGlobal("VP_Erevain_RaiseDe","GLOBAL",4) EscapeAreaMove("TT2114",152,150,14)~ EXIT
END
...
IF ~~ THEN BEGIN ErevainRessurection260
  ...
  IF ~~ THEN DO ~SetGlobal("VP_Erevain_RaiseDe","GLOBAL",4) SetGlobal("KickedOut","LOCALS",0) JoinParty()~ EXIT
END