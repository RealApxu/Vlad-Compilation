...
IF ~Global("VP_Guild_Attacked","GLOBAL",0) Global("PlayerAttackedAran","GLOBAL",0)~ THEN BEGIN 0
  SAY ~At them, lads! We'll not let these worthless scags take away our night!~
  IF ~Global("WorkingForBodhi","GLOBAL",1)~ THEN GOTO 1
  IF ~Global("WorkingForBodhi","GLOBAL",0) Global("WorkingForAran","GLOBAL",0)~ THEN GOTO 2
...
IF ~~ THEN BEGIN 1
  ...
  IF ~~ THEN DO ~SetGlobal("AranFleeVamp","GLOBAL",1) ActionOverride("VVRYOKOI",EscapeArea()) ActionOverride("VVCAT",EscapeArea()) EscapeArea()~ EXIT
END

IF ~OR(2) Global("VP_Guild_Attacked","GLOBAL",1) Global("PlayerAttackedAran","GLOBAL",1)~ THEN BEGIN 4
