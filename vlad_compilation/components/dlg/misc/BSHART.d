...
IF ~~ THEN BEGIN LeiSpher1
  ...
  IF ~InParty("Dar") !Dead("Dar")~ THEN EXTERN ~BDAR~ LeiSpher2
  IF ~InParty("Melora") !Dead("Melora")~ THEN EXTERN ~BMELORA~ LeiSpher1
  IF ~InParty("Taffic") !Dead("Taffic")~ THEN EXTERN ~BTAFFIC~ LeiSpher1
END
...
IF ~~ THEN BEGIN halruaa3
  ...
  IF ~InParty("Melora") !Dead("Melora")~ THEN EXTERN ~BMELORA~ halrustb
  IF ~OR(2) !InParty("Melora") Dead("Melora")~ THEN REPLY ~As appealing as it is, we'd have to take the academy in one fell swoop and we don't have enough fire power to do that. As for confronting a senior mage, he'd chew us up and spit us out in bubbles before we got near him. Bad ideas both of them.~ GOTO halruaa4
  IF ~OR(2) !InParty("Melora") Dead("Melora")~ THEN REPLY ~You really are insane girl! Take on a senior mage, raze the academy! Can't you feel the power pulsing through this place? it would become a hundred fold stronger should we raise arms against it! Bad plan Shar-Teel, very bad plan!~ GOTO halruaa4
  IF ~OR(2) !InParty("Melora") Dead("Melora")~ THEN REPLY ~Both are very bad ideas. I'll not risk the group's survival because you can not control your blood thirsty tendencies!~ GOTO halruaa4
  IF ~OR(2) !InParty("Melora") Dead("Melora")~ THEN REPLY ~You surprise me, I thought your first option would be the extent of your taste, one to one with the mage, just to see how much blood you can force out of him before he dies?~ GOTO halruaa5
END
